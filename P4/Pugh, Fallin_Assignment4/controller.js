    ...
    function createQuote() {
        ...
        if (gallons && devDate && devLocation && devPerName
            ...
            if (validFieldList.length === 8) {
                // TODO: make sure first ID is just auto, set current date to current date, get city, state, zip from user
                // TODO: Also get ID from session storage
                ...
                quoteToSend = new Quote('NextID', 1, validFieldList[1], today, validFieldList[5], validFieldList[0],
                    'City', 'TX', '78666', validFieldList[4], validFieldList[6], validFieldList[7],
                    validFieldList[3], validFieldList[2]);

                // const Http = new XMLHttpRequest();
                // const url = 'http://localhost:3000/api/fuelquotes/' + JSON.stringify(quoteToSend);
                // Http.open("POST", url);
                // Http.send();
    ...
    function displayClient(id = 1) {
        ...
        Http.onreadystatechange = function () {
            if (this.readyState == 4 && parsedJSON === '') {
                ...
                // TODO: Store this in session storage or something maybe just store client id in a cookie
                // As per DF, maybe store the whole client object in session storage.
                // Will cause issues with people starting at a page other than ClientInfo
                ...
                document.getElementById("clientInfoPhone").innerHTML = parsedJSON[0].phone;
...
{ // Function that is run when quoteHistory is loaded
        function populateQuoteHistory(id = 1) {
            const Http = new XMLHttpRequest();
